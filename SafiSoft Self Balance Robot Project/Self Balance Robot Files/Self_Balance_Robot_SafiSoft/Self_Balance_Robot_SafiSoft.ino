...
float MOTORSLACK_A = 100 ;                   // Compensate for motor slack range (low PWM values which result in no motor engagement)40
float MOTORSLACK_B = 100 ;
#define BALANCE_PID_MIN -255            // Define PID limits to match PWM max in reverse and foward 255
#define BALANCE_PID_MAX 255

MPU6050 mpu;

const int txpin = 5;       //Bluetooth serial stuff
const int rxpin = 6;
...
#define Kp  90    //Set this first 
#define Kd  3   //.Set this secound 
#define Ki  500   //Finally set this 

#define RKp  55 //Set this first 
...
void setup() {
 ...
 // Serial.begin(9600);
  ...
  originalSetpoint = 182   ;  //consigne 164
  yoriginalSetpoint = 173.5;
...
void new_pid() {
  ...
  MotorAspeed = compensate_slack(youtput, output, 1);//1
...
void Bt_control() {
  ...
  if (blue.available()) {
    ...
    if (content == 'F'){
      setpoint = originalSetpoint - d_speed; }            //forward
      
    else if (content == 'B'){
      setpoint = originalSetpoint + d_speed;}            //backward
